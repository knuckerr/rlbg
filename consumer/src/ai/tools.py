...
async def write_csv(
    ...
    try:
        await logger.log("INFO", f"execute write_csv with {data}")
        ...
        await logger.log("ERROR", f"error execute write with csv {e}")
        return f"Error: Failed to write to CSV {e}"


async def search_web_with_content(
    query: str, num_results: int = 10
) -> List[Dict[str, Any]]:
    try:
        await logger.log("INFO", f"execute search_web with {query}")
        ...
        await logger.log("ERROR", f"Error executing search with exa {e}")
        return []


async def write_json(filename: str, data: Any) -> str:
    output_dir: Path = Path("./output")
    output_dir.mkdir(parents=True, exist_ok=True)  # ensure folder exists
    await logger.log("INFO", f"execute write_json with {data}")
    try:
        filepath = output_dir / filename
        if not data:
            await logger.log("INFO", f"No data to call write_json")
            return "No data to write in the file"

        with filepath.open("w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
            await logger.log("INFO", f"Successfully wrote JSON to {filepath.resolve()}")
            return f"Json successfully written to {filepath.resolve()}"

    except Exception as e:
        await logger.log("ERROR", f"Error writing JSON: {e}")
        return f"Error: Failed to write JSON {e}"


async def write_text(filename: str, data: str) -> str:
    output_dir: Path = Path("./output")
    output_dir.mkdir(parents=True, exist_ok=True)  # ensure folder exists
    await logger.log("INFO", f"execute write_text with {data}")
    try:
        filepath = output_dir / filename
        if not data:
            await logger.log("INFO", f"No data to call write_text")
            return "No data to write in the file"

        with filepath.open("w", encoding="utf-8") as f:
            f.write(data)
            await logger.log("INFO", f"Successfully wrote text to {filepath.resolve()}")
            return f"Text successfully written to {filepath.resolve()}"

    except Exception as e:
        await logger.log("ERROR", f"Error writing text: {e}")
